...
public:
    ...
    EFetchResult DoCalculate(NUdf::TUnboxedValue& state, TComputationContext& ctx, NUdf::TUnboxedValue*const* output) const
    {
        ...
        if (s.Current_ == s.BlockLength_) do {
            if (const auto result = Flow_->FetchValues(ctx, fields); result != EFetchResult::One)
                return result;

            s.Current_ = 0;
            s.BlockLength_ = GetBlockCount(s.Values_.back());
            s.Transpose(fields);
        } while (!s.BlockLength_);
...
private:
    struct TState : public TComputationValue<TState> {
        ...
        void Transpose(NUdf::TUnboxedValue*const* fields) {
            for (int i = 0; i < Values_.size() - 1; ++i) {
                const ui8* byte_buffer = TArrowBlock::From(fields[i]).GetDatum().array()->template GetValues<uint8_t>(1);
                size_t item_size = CalcMaxBlockItemSize(fields[i]);
                // TODO: 1) apply MergeColumns,
                // 2) Convert output of MergeColumns to table of TUnboxedValues and
                // 3) save this table of TUnboxedValues in state
                // Table == vector of wide
            }
        }

        NUdf::TUnboxedValuePod Get(size_t idx) const {
            // TODO
            // Get single TUnboxedValue
            // May be we can give Wide row instead of single values
        }
    };
    ...
    void MakeState(NUdf::TUnboxedValue& state) const {
        state = ctx.HolderFactory.Create<TState>(Types_);
    }

    TState& GetState(NUdf::TUnboxedValue& state) const {
        if (!state.HasValue()) {
            MakeState(state);
        }
...
// TODO: unimplemented
// template<bool LLVM>
// TRuntimeNode MakeFromBlocks2(TSetup<LLVM>& setup) {
//     TProgramBuilder& pb = *setup.PgmBuilder;
//     TCallableBuilder callableBuilder(*setup.Env, "TestFromBlocks2",
//                                      pb.NewFlowType(
//                                          pb.NewMultiType({
//                                              pb.NewBlockType(pb.NewDataType(NUdf::EDataSlot::Uint64), TBlockType::EShape::Many),
//                                              pb.NewBlockType(pb.NewDataType(NUdf::EDataSlot::Uint64), TBlockType::EShape::Scalar),
