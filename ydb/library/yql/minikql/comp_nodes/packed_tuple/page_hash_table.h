...
class TPageHashTableImpl final: public TPageHashTable {
public:
    ...
    void Build(const ui8* data, const ui8 *const overflow, ui32 nItems) override final;

    ui32 FindMatches(const TTupleLayout* layout, const ui8* data, const std::vector<ui8, TMKQLAllocator<ui8>>& overflow, ui32 nItems) override final;
...
private:
    ...
    const ui8*                              OriginalOverflow_; // Overflow data passed to build method
};
...
void TPageHashTableImpl<TTraits>::Apply(const ui8 *const row, const ui8 *const overflow, auto &&onMatch) {
    ...
    while (true) { // Check all possible pages in collisions cycle
        ...
        while (foundBitMask != 0) { // Check all possible matches in single page
            ...
            bigKeySize: {
                if (CompareKeys(Layout_, tupleAddr, OriginalOverflow_, Layout_, row, overflow)) {
                    onMatch(tupleAddr);
                }
                continue;
            }
