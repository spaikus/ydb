...
#undef PackPOTColumn
    ...
    template <typename TTraits>
    void TTupleLayoutFallback<TTraits>::Unpack(ui8** columns, ui8** isValidBitmask, const ui8* res, const std::vector<ui8, TMKQLAllocator<ui8>>& overflow, ui32 start, ui32 count) const {
        std::vector<ui8*> columns_current;
        for (ui32 i = 0, next = 0; i < Columns.size(); ++i) {
            columns_current.emplace_back(columns[next++]);
            if (Columns[i].SizeType == EColumnSizeType::Variable) {
                columns_current.emplace_back(columns[next++]);
            }
        }

        for (auto& col: VariableColumns_) {
            WriteUnaligned<ui32>(columns_current[col.OriginalIndex], 0);
        }

        res += start * TotalRowSize;
        for (ui32 r = 0; count--; res += TotalRowSize, ++r) {
            auto bitmaskIdx = r / 8;
            auto bitmaskShift = r % 8;

            for (ui32 i = 0; i < Columns.size(); ++i) {
                auto& col = Columns[i];
                isValidBitmask[col.OriginalIndex][bitmaskIdx] |= ((res[BitmaskOffset + (i / 8)] >> (i % 8)) & 1u) << bitmaskShift;
            }

            for (auto& col: FixedNPOTColumns_) {
                std::memcpy(columns_current[col.OriginalIndex], res + col.Offset, col.DataSize);
                columns_current[col.OriginalIndex] += col.DataSize;
            }

#define PackPOTColumn(POT) \
            for (auto& col: FixedPOTColumns_[POT]) { \
                std::memcpy(columns_current[col.OriginalIndex], res + col.Offset, 1u << POT); \
                columns_current[col.OriginalIndex] += (1u << POT); \
            }
            PackPOTColumn(0);
            PackPOTColumn(1);
            PackPOTColumn(2);
...
#undef PackPOTColumn
            ...
            for (auto &col: VariableColumns_) {
                auto data = columns_current[col.OriginalIndex + 1];
                ...
                if (size < 255) { // embedded str
                    ...
                    auto prefixSize = (col.DataSize - 1 - 2 * sizeof(ui32));
                    auto overflowOffset = ReadUnaligned<ui32>(res + col.Offset + 1 + 0 * sizeof(ui32));
                    auto overflowSize = ReadUnaligned<ui32>(res + col.Offset + 1 + 1 * sizeof(ui32));

                    std::memcpy(data, res + col.Offset + 1 + 2 * sizeof(ui32), prefixSize);
                    std::memcpy(data + prefixSize, overflow.data() + overflowOffset, overflowSize);

                    size = prefixSize + overflowSize;
                }

                auto current_offset = *columns_current[col.OriginalIndex] + size;
                columns_current[col.OriginalIndex] += sizeof(ui32);
                WriteUnaligned<ui32>(columns_current[col.OriginalIndex], current_offset);
                columns_current[col.OriginalIndex + 1] += size;
            }
        }
    }
